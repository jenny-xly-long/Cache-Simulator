def mxm_blocked(myCPU, d, f, p, WORD_SIZE):
    ...
    for jj in range(0,d,f):
        for kk in range(0,d,f):
            for i in range(d):
                for j in range(jj, min(jj+f,d)):
                    register0 = 0
                    for k in range(kk, min(kk+f,d)):
                    ...
                    myCPU.storeDouble(c[i][j],myCPU.loadDouble(c[i][j])+register0)
